...
dat_ccf <- loadRDS("data/ccf.rds")

## insert values
rst_lag <- rst_cor <- rst_chirps[[1]]
rst_lag[] <- rst_cor[] <- NA

## colors
cols <- colorRampPalette(brewer.pal(9, "RdBu"))
cols_lag <- brewer.pal(9, "YlOrRd")[c(1, 4, 6, 9)]

lst_ccf <- foreach(i = 1:12, month = month.abb, .packages = "Rsenal") %dopar% {
  id_month <- seq(i, nrow(dat_ccf), 12)

  rst_cor_tmp <- raster::setValues(rst_cor, dat_ccf$r[id_month])
  ...
  rst_lag_tmp <- raster::setValues(rst_lag, dat_ccf$lag[id_month])
